...
static const char kKeyChainUDIDAccessGroup[] = "com.eall.beaver";
...
+ (NSString*)getUDIDFromKeyChain
{
    ...
    NSString *udid      = nil;
    CFTypeRef cfUdidValue = (__bridge CFTypeRef)udidValue;
    queryErr = SecItemCopyMatching((__bridge CFDictionaryRef)dictForQuery, &cfUdidValue);

    NSMutableDictionary *dict = nil;
    CFTypeRef cfDict = (__bridge CFTypeRef)dict;
    [dictForQuery setValue:(__bridge id)kCFBooleanTrue forKey:(__bridge id)kSecReturnAttributes];
    queryErr = SecItemCopyMatching((__bridge CFDictionaryRef)dictForQuery, &cfDict);
...
+ (BOOL)updateUDIDInKeyChain:(NSString*)newUDID
{
    ...
    CFTypeRef cfQuery = (__bridge CFTypeRef)queryResult;
    SecItemCopyMatching((__bridge CFDictionaryRef)dictForQuery, &cfQuery);
    if (queryResult) {
