...
//        [self.view addSubview:[self buildHeaderView]];
//        [[[UIApplication sharedApplication] keyWindow] setBackgroundColor:[UIColor whiteColor]];
        
        // Navigation Bar Items
        UIBarButtonItem *doneButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"下一步"
                                                                           style:UIBarButtonItemStylePlain target:self action:@selector(doneAction:)];
        [doneButtonItem setBackgroundImage:[[UIImage alloc] init] forState:UIControlStateNormal barMetrics:UIBarMetricsDefault];
...
- (UIView*)buildHeaderView:(CGRect)frame
{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil) 
    {		        
        cell = [[AGIPCGridCell alloc] initWithImagePickerController:self.imagePickerController assetsController:self items:[self itemsForRowAtIndexPath:indexPath] andReuseIdentifier:CellIdentifier];
    }	
	else 
    {		
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    self.tableView.frame = CGRectOffset(frame, 0, 30);
    
    UIView *faView = self.tableView.superview;
    frame = faView.frame;
    
    CGRect navBarFrame = self.navigationController.navigationBar.frame;
    [faView addSubview:[self buildHeaderView:CGRectMake( 0, navBarFrame.origin.y + navBarFrame.size.height, self.view.width, 30)]];
//    [[[UIApplication sharedApplication] keyWindow] addSubview:[self buildHeaderView:CGRectMake( 0, navBarFrame.origin.y + navBarFrame.size.height, 320, 30)]];
}
...
- (void)loadAssets
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        ...
        AGIPCAssetsController *strongSelf = weakSelf;
...
//                [strongSelf.assets insertObject:gridItem atIndex:0];
                ...
                [strongSelf.assets insertObject:result atIndex:0];
            }];
        }
