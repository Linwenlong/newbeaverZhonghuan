...
- (void)setSelected:(BOOL)selected
{
    @synchronized (self)
    {
        if (_selected != selected)
        {
            if (selected) {
                ...
                if ([self.delegate respondsToSelector:@selector(agGridItemCanSelect:)])
                {
                    if (![self.delegate agGridItemCanSelect:self])
                    {
                        ...
                        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:title message:NSLocalizedString(@"photo_add_warn", nil) delegate:self cancelButtonTitle:NSLocalizedString(@"photo_add_confirm", nil) otherButtonTitles:nil];
                        [alertView show];
                        return;
                    }
                }
            ...
            __weak AGIPCGridItem *weakSelf = self;
            
            dispatch_async(dispatch_get_main_queue(), ^{
               
                if ([weakSelf.delegate respondsToSelector:@selector(agGridItem:didChangeSelectionState:)])
                {
                    [weakSelf.delegate performSelector:@selector(agGridItem:didChangeSelectionState:) withObject:weakSelf withObject:@(_selected)];
                }
                
                if ([weakSelf.delegate respondsToSelector:@selector(agGridItem:didChangeNumberOfSelections:)])
                {
                    [weakSelf.delegate performSelector:@selector(agGridItem:didChangeNumberOfSelections:) withObject:weakSelf withObject:@(numberOfSelectedGridItems)];
                }
...
- (void)dealloc
{
    ...
    if (_checkmarkImageView) {
        ...
        [_checkmarkImageView setImage:nil];
