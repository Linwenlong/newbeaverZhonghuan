...
-(id)initWithTitle:(NSString *)inTitle buttons:(NSMutableArray *)buttons
{
...
//        self.subviews
        ...
        [buttons addObject:[RIButtonItem itemWithLabel:NSLocalizedString(@"cancel", nil)]];
        NSInteger cancelIndex = [self addButtonWithTitle:NSLocalizedString(@"cancel", nil)];
        [self setCancelButtonIndex:cancelIndex];

        objc_setAssociatedObject(self, (__bridge const void *)RI_BUTTON_ASS_KEY, buttons, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    return self;
}
