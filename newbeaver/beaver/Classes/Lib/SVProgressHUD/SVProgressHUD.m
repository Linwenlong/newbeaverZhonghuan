...
+ (void)showSuccessWithStatus:(NSString *)string maskType:(SVProgressHUDMaskType)maskType
{
    ...
    [SVProgressHUD dismissWithSuccess:string afterDelay:3];
}
...
+ (void)dismiss:(void(^)(BOOL finished))completion{
	[[SVProgressHUD sharedView] dismiss:completion];
}
...
- (void)dismissWithStatus:(NSString *)string error:(BOOL)error afterDelay:(NSTimeInterval)seconds {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(error)
            self.imageView.image = [UIImage imageNamed:@"alert_error"];
        else
            self.imageView.image = [UIImage imageNamed:@"alert_success"];
