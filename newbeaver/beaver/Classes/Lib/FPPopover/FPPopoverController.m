...
-(void)presentPopoverFromNavigationBtn:(UIView*)fromView
{
    SAFE_ARC_RELEASE(_fromView);
    _showFromNavigationBtn = YES;
    _fromView = SAFE_ARC_RETAIN(fromView);
    [self presentPopoverFromPoint:[self originFromView:_fromView]];
}
...
-(CGRect)bestArrowDirectionAndFrameFromView:(UIView*)v
{
    ...
    self.origin = CGPointMake(p.x + v.frame.size.width/2.0, p.y + (_showFromNavigationBtn?0:v.frame.size.height/2.0));
