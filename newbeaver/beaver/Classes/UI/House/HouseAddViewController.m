...
@interface HouseAddViewController ()<UIAlertViewDelegate>
{
    UIView *footerView;
...
@implementation HouseAddViewController
...
- (void)viewDidLoad
{
    ...
    if (self.editFlag) {
        self.navigationItem.title = NSLocalizedString(@"house_edit_detail_title", nil);
        
        NSString *type = self.house.rentalState == EHouseRentalTypeRent ? @"rent" : (self.house.rentalState == EHouseRentalTypeSale ? @"sale": @"sale_rent");
        NSDictionary *params = @{@"house_id": self.house.id, @"type": type, @"chunk":[NSNumber numberWithInt:3]};
        ...
        [[EBHttpClient sharedInstance] houseRequest:@{@"type": self.params[@"type"], @"purpose": self.params[@"purpose"]} houseParameter:^(BOOL success, id result) {
            NSLog(@"resut = %@",result);
            if (!success) {
                return;
            }
            ...
            if (self.if_start == YES) {//开启了座栋规则
                if (self.if_lock == YES) {//房号锁定  多了面积 跟 房型
                    [self initParserContainer:result lifts:_lists rooms:_rooms floor:_floor totleFloor:_totle_floor room:_room living_room:_living_room washroom:_washroom balcony:_balcony area:_usable_area];
                }else{//梯 户 楼层 总楼层
                    [self initParserContainer:result lifts:_lists rooms:_rooms floor:_floor totleFloor:_totle_floor];
                }
            }else{//没有开启座栋规则
                [self initParserContainer:result];
            }
            
//            [self initParserContainer:result lifts:3 rooms:4];
...
- (void)initFooterView
{
    if (footerView) {
        ...
        else
        {
            ...
            footerView.frame = CGRectMake(0, self.parserContainerView.height + 10, footerView.width, 0);
            [self resizeContentSize];
        }
...
//    __weak HouseAddViewController *weakSelf = self;
    [[EBHttpClient sharedInstance] houseRequest:params houseSave:^(BOOL success, id result) {
        ...
        if (![dic.allKeys containsObject:@"id"]) {
            //返回到初始界面
            UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"提示" message:dic[@"desc"] delegate:self cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
            [alertView show];
            return ;
        }
        ...
        if (success) {
            ...
            [[EBHttpClient sharedInstance] houseRequest:@{@"id":house.id, @"force_refresh":@(YES),
                                                 ...
                                                 detail:^(BOOL success, id result)
             {
                 ...
                 NSLog(@"resultFinish = %@",result);
                 
                 if (success)
                 {
                     house = result;
                     HouseAddFinishViewController *viewController = [[HouseAddFinishViewController alloc] init];
                     viewController.house = house;
                     [self.navigationController pushViewController:viewController animated:YES];
                 }
             }];
            
//            HouseAddViewController *strongSelf = weakSelf;
//            NSString *mes = NSLocalizedString(@"house_add_photo_hint", nil);
//            [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"house_add_alert_text_success", nil) message:mes cancelButtonItem:[RIButtonItem itemWithLabel:NSLocalizedString(@"cancel", nil) action:^{
//                [strongSelf.navigationController popToRootViewControllerAnimated:YES];
//            }] otherButtonItems:[RIButtonItem itemWithLabel:NSLocalizedString(@"confirm", nil) action:^{
//                //add photo
//                [self getPhoto:house];
//            }], nil] show];
        } else {
...
- (void)endEdit
{
    ...
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithObjectsAndKeys:self.house.id, @"house_id", type, @"type", [NSNumber numberWithInt:3], @"chunk", nil];
    params = [self setReqParams:params];
    if (!params) {
        return;
    }
    ...
    [self.currentElementView deSelect:nil];
    [EBAlert showLoading:nil];
    __block HouseAddViewController *weakSelf = self;
