...
@interface ZYSideSlipFilterController () <UITableViewDelegate, UITableViewDataSource, SideSlipBaseTableViewCellDelegate,HooDatePickerDelegate>
...
@property (weak, nonatomic) ZYSideSlipFilterRegionModel *regionModel;
@property (weak, nonatomic) SideSlipPriceTableViewCell *priceCell;

@property (nonatomic, weak)SideSlipAllCategoryTableViewCell *slipCell;

@property (assign, nonatomic) NSInteger current_btn;

@property (strong, nonatomic) NSMutableDictionary *tmpDic;

@property (nonatomic, strong)NSDate * tmpStartDate;
@property (nonatomic, strong)NSDate * tmpEndDate;
...
- (instancetype)initWithSponsor:(UIViewController *)sponsor
    ...
    if (self) {
        ...
        [self.tmpDic setObject:@"0" forKey:@"start"];
        [self.tmpDic setObject:@"0" forKey:@"end"];
        _sponsor = sponsor;
...
- (UIView *)createBottomView {
    ...
    [resetButton setBackgroundColor:UIColorFromRGB(0x019CFF)];
    ...
    [commitButton setBackgroundColor:UIColorFromRGB(0xfe3800)];
...
- (void)clickResetButton:(id)sender {
    ...
    [self dismiss];
    self.slipCell.controlLabel.text = @"全部";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([cell isKindOfClass:[SideSlipPriceTableViewCell class] ]) {
        SideSlipPriceTableViewCell *tmpCell = (SideSlipPriceTableViewCell*)cell;
        tmpCell.start = @" ";
        tmpCell.end = @" ";
        tmpCell.current_path = indexPath;
        
        self.priceCell = tmpCell;
        self.regionModel = tmpCell.regionModel;
        
        [tmpCell.startBtn addTarget:self action:@selector(showTest:) forControlEvents:UIControlEventTouchUpInside];
        [tmpCell.endBtn addTarget:self action:@selector(showTest:) forControlEvents:UIControlEventTouchUpInside];
        NSLog(@"测试");
    }else if ([cell isKindOfClass:[SideSlipAllCategoryTableViewCell class] ]){
        self.slipCell = (SideSlipAllCategoryTableViewCell*)cell;
    }
...
- (void)showTest:(UIButton *)btn{
    
    _current_btn = btn.tag;
...
- (void)datePicker:(HooDatePicker *)dataPicker didSelectedDate:(NSDate *)date{
    ...
    if ([date compare:[NSDate date]] < 0) {
        ...
        [EBAlert alertError:@"请选择小于本日的日期" length:2.0 ];
        return;
    }
    ...
    if (_current_btn == 0) {
        if (_tmpEndDate && [date compare:_tmpEndDate] > 0) {
            [EBAlert alertError:@"请选择小于结束的日期" length:2.0 ];
            return;
        }
        _tmpStartDate = date;
        [self.priceCell.startBtn setTitle:currentDateString forState:UIControlStateNormal];
        self.priceCell.start = currentDateString;
    }else{
        if (_tmpStartDate && [date compare:_tmpStartDate] < 0) {
            [EBAlert alertError:@"请选择大于开始的日期" length:2.0 ];
            return;
        }
        _tmpEndDate = date;
        [self.priceCell.endBtn setTitle:currentDateString forState:UIControlStateNormal];
        self.priceCell.end = currentDateString;
    }
    
    ZYSideSlipFilterRegionModel *model = _dataList[_priceCell.current_path.row];
    [self.priceCell updateCellWithModel:&model indexPath:_priceCell.current_path];
}

- (HooDatePicker *)datePicker{
    if (!_datePicker) {
        _datePicker = [[HooDatePicker alloc] initWithSliderSuperView:self.view];
