...
- (void)fitCollectonViewHeight {
    NSLog(@"heitht = %ld",self.dataList.count);
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.tag == 0) {
        if (_regionModel.selectedItemList.count != 0) {
            [[NSUserDefaults standardUserDefaults] setObject:@1 forKey:@"isSelectedStaff"];//不能选员工
            [[NSNotificationCenter defaultCenter] postNotificationName:@"clearStaff" object:nil];
        }else{
            [[NSUserDefaults standardUserDefaults] setObject:@0 forKey:@"isSelectedStaff"];//能选员工
        }
        
    }
    NSLog(@"self.tag = %d",_regionModel.isShowAll);
    NSLog(@"self.tag = %@",_regionModel.selectedItemList);
    [_mainCollectionView reloadData];
}
