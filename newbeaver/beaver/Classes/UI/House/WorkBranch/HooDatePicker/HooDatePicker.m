...
static NSString  * const kClearButtonItemTitle = @"清空";
...
//static CGFloat const kHooDatePickerLineWidth = 2.0f;//之前的为2.0 lwl
...
// Constants colors :UIColor *lableColor = UIColorFromRGB(0xff3800);
...
//#define kHooDatePickerHighlightColor UIColorFromRGB(0xff3800)//选择的高亮红色
#define kHooDatePickerHighlightColor   [UIColor blackColor]
...
//是否显示标题
- (instancetype)initWithSuperView:(UIView*)superView withTitle:(NSString *)title{
...
//是否显示清空按钮
- (instancetype)initWithSuperView:(UIView*)superView withIsShowClearBtn:(BOOL)isShowClearBtn{
    if (self = [super initWithFrame:CGRectMake(0.0, superView.frame.size.height, superView.frame.size.width, kHooDatePickerHeight)]) {
...
- (instancetype)initWithSliderSuperView:(UIView*)superView{
    if (self = [super initWithFrame:CGRectMake(0.0, superView.frame.size.height, superView.frame.size.width, kHooDatePickerHeight)]) {
        _isShowClearBtn = NO;
        _datePickerMode = HooDatePickerModeDate;
        [superView addSubview:self];
...
//     self.backgroundColor = [UIColor redColor];
    ...
    if (self.datePickerMode == HooDatePickerModeTen) {
        //        _days = [NSMutableArray arrayWithArray:@[@"上半月",@"下半月"]];
        [_days removeObjectsInRange:NSMakeRange(2, _days.count-3)];
        NSLog(@"_days = %@",_days);
        
        _tmp = [NSMutableArray arrayWithArray:@[@"月上旬",@"月中旬",@"月下旬"]];
    ...
    //up down Seleectors
    if (self.datePickerMode == HooDatePickerModeUpAndDown) {
//        _days = [NSMutableArray arrayWithArray:@[@"上半月",@"下半月"]];
        [_days removeObjectsInRange:NSMakeRange(2, _days.count-2)];
        NSLog(@"_days = %@",_days);
        
        _tmp = [NSMutableArray arrayWithArray:@[@"上半月",@"下半月"]];
...
//    [self setDate:[self getEailer:[NSDate date]] animated:NO];
...
- (void)buildSelectorLabelsDays {
    ...
    for (int i = 0; i < _days.count; i++) {
        NSString *day = nil;
        if (self.datePickerMode == HooDatePickerModeUpAndDown||self.datePickerMode == HooDatePickerModeTen) {
            day = (NSString*)[_tmp objectAtIndex:i];
        }else{
            day = (NSString*)[_days objectAtIndex:i];
        }
...
- (void)actionButtonValid:(UIButton *)sender {
    ...
    if (self.datePickerMode == HooDatePickerModeUpAndDown||self.datePickerMode == HooDatePickerModeTen) {
        if ([self.delegate respondsToSelector:@selector(datePicker:didSelectedMonth:andUpOrDown:)]) {
            [self.delegate datePicker:self didSelectedMonth:[self getStr] andUpOrDown:[self getType]];
        }
    }else{
...
- (void)actionButtonClear:(UIButton *)sender{
    ...
    if ([self.delegate respondsToSelector:@selector(datePicker:didClear:)]) {
        [self.delegate datePicker:self didClear:sender];
    }
}
...
//    NSLog(@"data = %ld",(long)self.datePickerMode);
    ...
    if (self.datePickerMode == HooDatePickerModeUpAndDown|| self.datePickerMode == HooDatePickerModeTen) {
         ...
         NSLog(@"index = %ld",_selectedYear - _minYear);
    ...
    NSLog(@"_selectedYear=%ld",_selectedYear-1900);
    if (self.datePickerMode == HooDatePickerModeYear) {
        
        [self setScrollView:_scrollViewYears atIndex:(_selectedYear - 1900) animated:animated];
    }
...
- (NSString*)convertToDateDay:(NSInteger)day month:(NSInteger)month year:(NSInteger)year{
...
- (UIView *)headerView {
    if (!_headerView) {
        ...
        [_headerView addSubview:cancelButton];
          ...
          //Button clear
          if (_isShowClearBtn == YES) {
              UIButton *clearButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width - kHooDatePickerButtonHeaderWidth - kHooDatePickerPadding, 0.0, kHooDatePickerButtonHeaderWidth, kHooDatePickerHeaderHeight)];
              clearButton.center = _headerView.center;
              [clearButton setTitle:kClearButtonItemTitle forState:UIControlStateNormal];
              [clearButton setTitleColor:self.highlightColor forState:UIControlStateNormal];
              [clearButton addTarget:self action:@selector(actionButtonClear:) forControlEvents:UIControlEventTouchUpInside];
              [_headerView addSubview:clearButton];
            }
...
- (NSTimeZone *)timeZone {
    if (!_timeZone) {
        _timeZone = [NSTimeZone timeZoneWithAbbreviation:@"zh_CN"];
    }
    return _timeZone;
}
