//
//  MortgageYiJieZhangTableViewCell.m
//  dev-beaver
//
...
@interface MortgageYiJieZhangTableViewCell ()
...
@property (nonatomic, strong)UILabel * confirNameTitle;
@property (nonatomic, strong)UILabel * confirNameContent; //确认人

@property (nonatomic, strong)UILabel * confirTimeTitle;
@property (nonatomic, strong)UILabel * confirTimeContent; //确认时间

@property (nonatomic, strong)UILabel * confir; //确认

@end

@implementation MortgageYiJieZhangTableViewCell
...
-(void)setDic:(NSDictionary *)dic{
    ...
    _confirNameContent.text = [NSString stringWithFormat:@"%@ %@",dic[@"confirm_dept_name"],dic[@"confirm_username"]];
    _confirTimeContent.text = [NSString timeWithTimeIntervalString:dic[@"confirm_date"] format:@"yyyy-MM-dd hh: mm"];
...
- (void)setUI{
    ...
    _line.backgroundColor = RGBA(217, 217, 217, 1);
    
    _confirNameTitle = [UILabel new];
    _confirNameTitle.textAlignment = NSTextAlignmentLeft;
    _confirNameTitle.text = @"确认人:";
    _confirNameTitle.textColor = UIColorFromRGB(0x000000);
    _confirNameTitle.font = [UIFont systemFontOfSize:14.0f];
    
    _confirNameContent = [UILabel new];
    _confirNameContent.textAlignment = NSTextAlignmentRight;
    _confirNameContent.text = @"IT部  徐某某";
    _confirNameContent.textColor = UIColorFromRGB(0x474747);
    _confirNameContent.font = [UIFont systemFontOfSize:13.0f];
    
    _confirTimeTitle = [UILabel new];
    _confirTimeTitle.textAlignment = NSTextAlignmentLeft;
    _confirTimeTitle.text = @"确认时间:";
    _confirTimeTitle.textColor = UIColorFromRGB(0x000000);
    _confirTimeTitle.font = [UIFont systemFontOfSize:14.0f];
    
    _confirTimeContent = [UILabel new];
    _confirTimeContent.textAlignment = NSTextAlignmentRight;
    _confirTimeContent.text = @"2018-12-10  20：10";
    _confirTimeContent.textColor = UIColorFromRGB(0x474747);
    _confirTimeContent.font = [UIFont systemFontOfSize:13.0f];
    
    
    
    _confir = [UILabel new];
    _confir.text = @"已结账";
    _confir.textAlignment = NSTextAlignmentRight;
    _confir.font = [UIFont systemFontOfSize:14.0f];
    _confir.textColor = RGBA(254,58,3,1);

    
    [self.contentView sd_addSubviews:@[_dept_name,_proportionType,_proportionPrice,_first_dept_name,_first_proportionType,_first_proportionPrice,_second_dept_name,_second_proportionType,_second_proportionPrice,_totle_dept_name,_totle_proportionType,_totle_proportionPrice,_confirNameTitle,_confirNameContent,_confirTimeTitle,_confirTimeContent,_confir,_line]];
...
- (void)addLayoutSubviews{
    ...
    _confirNameTitle.sd_layout
    .topSpaceToView(_line, 10)
    .leftSpaceToView(self.contentView, 15)
    .widthIs([self sizeToWith:[UIFont systemFontOfSize:16.0f] content:@"确认人:"])
    .heightIs(h);
    
    _confirNameContent.sd_layout
    .topSpaceToView(_line, 10)
    .rightSpaceToView(self.contentView, 15)
    .leftSpaceToView(_confirNameTitle, 20)
    .heightIs(h);
    
    _confirTimeTitle.sd_layout
    .topSpaceToView(_confirNameTitle, 17)
    .leftSpaceToView(self.contentView, 15)
    .widthIs([self sizeToWith:[UIFont systemFontOfSize:16.0f] content:@"确认时间:"])
    .heightIs(h);
    
    _confirTimeContent.sd_layout
    .topSpaceToView(_confirNameTitle, 17)
    .rightSpaceToView(self.contentView, 15)
    .leftSpaceToView(_confirTimeTitle, 20)
    .heightIs(h);
    
    _confir.sd_layout
    .topSpaceToView(_confirTimeTitle, 13)
    .rightSpaceToView(self.contentView, right)
    .widthIs(60)
    .heightIs(h);
}
