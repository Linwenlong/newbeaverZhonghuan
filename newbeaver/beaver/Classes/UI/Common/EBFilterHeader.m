...
//      _indicatorUp = [_indicatorDown imageRotatedByDegrees:180];
      ...
      _currentOpen = -1;
      NSInteger totalCount = _titles.count;
      CGFloat itemWidth = frame.size.width /(CGFloat) totalCount;
      CGFloat xOffset = 0;
      CGFloat separatorWidth = (frame.size.width - (totalCount - 1 ) * 1.0) / totalCount;
      for (NSInteger i = 0; i < totalCount; i++)
      {
          ...
          if (i < totalCount - 1)
          {
              [self addSeparator:separatorWidth * (i + 1) + i * 1];
          }
          xOffset += itemWidth;
      }
//      self.clipsToBounds = YES;
      [self setBackgroundColor:[UIColor whiteColor]];
//      [self setBackgroundColor:[UIColor colorWithRed:37.0/255.f green:58.0/255.f blue:103.0/255.f alpha:1.0]];
...
- (void)resizeIcLabel:(EBIconLabel *)icLabel index:(NSInteger)index
{
    ...
    CGFloat itemWidth = self.frame.size.width / _titles.count;
    if (title == nil)
    {
        title = _titles[index];
    }
...
{
    UIImageView *separator = [[UIImageView alloc] initWithFrame:CGRectMake(xOffset, 0, 1.0, self.frame.size.height)];
