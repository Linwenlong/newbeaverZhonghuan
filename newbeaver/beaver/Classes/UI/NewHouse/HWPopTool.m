...
@property (weak, nonatomic) MainButton *closeButton;
...
- (MyViewController*)getMainController{
   return [HWPopTool sharedInstance].viewController;
}
...
- (void)setCloseButtonType:(ButtonPositionType)closeButtonType {
    ...
    if (closeButtonType == ButtonPositionTypeNone) {
        ...
        if(closeButtonType == ButtonPositionTypeRight){
            closeFrame.origin.x = round(CGRectGetWidth(self.containerView.frame) - kDefaultCloseButtonPadding - CGRectGetWidth(closeFrame)/2)-8;
        } else {
...
- (MyViewController*)showWithPresentView:(UIView *)presentView animated:(BOOL)animated {
    ...
    MainButton *closeButton = [[MainButton alloc] init];
...
- (void)cleanup{
    ...
    self.window = nil;
    [[NSNotificationCenter defaultCenter]postNotificationName:@"kselect" object:nil];
}
