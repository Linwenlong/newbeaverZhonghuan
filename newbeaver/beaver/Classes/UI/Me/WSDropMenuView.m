...
- (void)_setButton{
    ...
    [self.leftButton setTitleColor:UIColorFromRGB(0x404040) forState:UIControlStateNormal];
    ...
    UIView *line = [[UIView alloc]initWithFrame:CGRectMake(CGRectGetMaxX(self.leftButton.frame), 0, 1, self.height)];
    line.backgroundColor = [UIColor colorWithRed:0.85 green:0.85 blue:0.85 alpha:1.00];
    ...
    [self.rightButton setTitleColor:UIColorFromRGB(0x404040)   forState:UIControlStateNormal];
...
- (void)buttonAction:(UIButton *)sender{
    ...
    if (self.rightButton == sender) {
        ...
        if (self.btnDelegate && [self.btnDelegate respondsToSelector:@selector(btnClick:)]) {
            [self.btnDelegate btnClick:self.rightButton];
        }
        return;
//        _isRightOpen = YES;
//       
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.leftTableView_2 == tableView) {
        ...
        _currString = cell.textLabel.text;
