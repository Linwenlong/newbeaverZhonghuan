...
int sizeof_int = sizeof(uintptr_t);
...
int ArrayReSize(Array* A)
{
	...
	if( !A->pIndexData )
		A->pIndexData = (uintptr_t**)malloc(mallocindexsize);
	
	newsize = A->mallocsize + MALLOC_SIZE;
	desData = (uintptr_t*)malloc(mallocsize);
...
void ArrayCSetSize(struct ArrayC* A,int size)
{
    ...
    A->pDataSize = size;
    A->pData = (uintptr_t*)malloc(size*sizeof_int);
	A->pDataEnd = A->pData;
}
